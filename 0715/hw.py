...
def age_df_sch(df):
    ...
    idx = df['구분'].unique() # 연령대별 마커를 만들기 위함

    for i in range(len(idx)): # 연령대 고유값을 통해 각각 마커 생성
        df1 = df[df['구분']==idx[i]]
...
if __name__ == '__main__':
    ...
    sDate = datetime.strptime(sDate, "%Y%m%d") + timedelta(days=-1) # 입력한 시작일 당일 확진자수를 계산하기 위해 전날 데이터까지 불러오기 위함
